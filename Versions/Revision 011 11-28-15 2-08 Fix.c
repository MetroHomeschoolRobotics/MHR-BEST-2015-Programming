...
#pragma config(Motor,  port4,           clawL,         tmotorServoStandard, openLoop)
#pragma config(Motor,  port5,           scooper,       tmotorServoStandard, openLoop)
...
// Cart rotates the manipulator in the front of the robot
		if(vexRT[Btn8L]) {
...
// Cart 2 is the servo finger in the front for RAF grabber :-)
		if(vexRT[Btn7U]) {
		...
		if(vexRT[Btn8D] && macro == false) {
			...
			wait1Msec(80);
			motor[arm] = 0;
		} else if(vexRT[Btn8D] && macro == false) {
			macro = true;
			motor[arm] = 75;
			wait1Msec(80);
			motor[arm] = 0;
		} else if(macro == true && !vexRT[Btn8D] && !vexRT[Btn8D]) {
			macro = false;
		}
